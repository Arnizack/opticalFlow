...
def setupImageForTracking(templateImgPath, targetImgPath ):
    ...
    gradT = np.empty(shape=(colorDim,2),dtype="O")

    for c in range(colorDim):
        I[c] = interpolate2dArray(targImg[:,:,c])
        T[c] = interpolate2dArray(templImg[:,:,c])
        gradX1 = grayScaleGradient(templImg[:,:,c],"X")
        gradX2 = grayScaleGradient(templImg[:,:,c],"Y")
        gradT[c][0]=interpolate2dArray(gradX1)
        gradT[c][1]=interpolate2dArray(gradX2)
    return (I,T,gradT)
