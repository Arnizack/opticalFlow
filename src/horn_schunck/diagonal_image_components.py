...
def get_img_diags(first_image, second_image):
    gray_first_image = convert_img_to_gray(first_image)
    gray_second_image = convert_img_to_gray(second_image)

    plt.title("gray_first_image")
    plt.imshow(gray_first_image)
    plt.figure()

    derivative_mode = "Horn-Schunck"
...
def get_img_diags_color(first_image, second_image):
