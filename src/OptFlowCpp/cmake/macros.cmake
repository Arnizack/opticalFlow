function(optflow_set_solution_folder target)
  #get USE_FOLDERS
...
macro(optflow_set_source_group source)
  #get USE_FOLDERS
  set(should_use_folder )
  get_property(should_use_folder GLOBAL PROPERTY USE_FOLDERS)
  message("Use Folder: ${should_use_folder}")
...
macro(__optflow_set_include_dirs target includes scope)
...
macro(optflow_link_dependencies target library_deps scope)
...
macro(__optflow_target_setup target sources includes library_deps scope)
  optflow_link_dependencies(${target} "${library_deps}" "${scope}")

  optflow_set_solution_folder(${target})
  optflow_set_source_group("${sources}")
  #source_group(TREE ${CMAKE_CURRENT_SOURCE_DIR} FILES ${sources})
  
  __optflow_set_include_dirs(${target} "${includes}" PUBLIC)
...
macro(optflow_add_library target sources includes library_deps lib_typ)
  ...
  __optflow_target_setup(${target} "${sources}" "${includes}" "${library_deps}" "")
  
endmacro()

macro(optflow_add_header_only_lib target includes library_deps)
  add_library(${target} INTERFACE)
  __optflow_set_include_dirs(${target} "${includes}" INTERFACE)
  optflow_link_dependencies(${target} "${library_deps}" INTERFACE)
  
endmacro()

macro(optflow_add_executable target sources includes library_deps )
  add_executable(${target} ${sources})
  __optflow_target_setup(${target} "${sources}" "${includes}" "${library_deps}" "")
  
endmacro()

macro(optflow_add_gtest target sources includes library_deps)
  add_executable(${target} ${sources})
  add_test("${target}" ${target})
  set(library_deps_edit "${library_deps}"  gmock_main gmock gtest_main gtest)
  #message("gtest library_deps_edit ${gtest_libs}")
  __optflow_target_setup(${target} "${sources}" "${includes}" "${library_deps_edit}" PRIVATE)
...
macro(optflow_add_gmock_lib target sources includes library_deps)
  ...
  __optflow_target_setup(${target} "${sources}" "${includes}" "${library_deps_edit}" PUBLIC)
