...
namespace cpu_backend
{
    void BilateralMedian(
        ...
        double sigma_color, int filter_length_half, size_t width, size_t height, size_t color_channel_count,
        double* destination)
    {
        size_t weights_max_length = filter_length_half * filter_length_half * 4;
        ...
        for (int y = 0; y < height; y++)
        {
            ...
            {
                int coord = y * width + x;
                destination[coord] = BilateralMedianAt(x, y, flow, auxiliary_flow, log_occlusion, image, weights,
                    median_list, filter_influence, auxiliary_influence, sigma_distance, sigma_color, filter_length_half,
        ...
        double sigma_distance, double sigma_color, double* destination)
    {
        ...
        size_t x_end = std::min<size_t>(x + filter_length_half, width - 1);
        size_t y_start = std::max(y - filter_length_half, 0);
        size_t y_end = std::min<size_t>(y + filter_length_half, height - 1);
        ...
        size_t color_channel_count)
    {
        size_t weights_count = BilateralMedianWeight(x, y, log_occlusion, image, width, height, color_channel_count,
            filter_length_half, sigma_distance, sigma_color, weights);
        _BilateralMedianList(x, y, flow, auxiliary_flow, filter_influence, auxiliary_influence, weights, filter_length_half,
            weights_count, width, height, median_list);
        return _inner::_median(median_list, weights_count * 2 + 1);
    }
