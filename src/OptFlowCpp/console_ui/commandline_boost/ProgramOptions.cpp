...
namespace console_ui
{
	...
	std::shared_ptr<Hypodermic::Container> ComnandlineSetup(int argc, char* argv[], std::string first_image_path, std::string second_image_path, std::string flow_output_path, std::string flow_img_output_path)
	{
		bpo::options_description generic_opt = SetupGenericOptions();

		bpo::options_description io_opt = SetupIOOptions(first_image_path, second_image_path, flow_output_path, flow_img_output_path);
		...
		return SetupCommandlineSolvers(input);
	}
