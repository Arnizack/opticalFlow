...
namespace cpu_backend
{
	namespace testing
	{
		...
		void TestForTypeAdd()
		{
			...
			std::shared_ptr<Array<T, dim>> test_obj;
			
			//Add
			test_obj = ArithmeticBasic<T, dim>::Add(in_a, in_b);
			test_obj.get()->CopyDataTo(ptr);
			control_add_sub<T>(ptr, arr_a, arr_b, 1, size, test_obj.get()->Size());
			//AddTo
			ArithmeticBasic<T, dim>::AddTo(test_obj, in_a, in_b);
			test_obj.get()->CopyDataTo(ptr);
			control_add_sub<T>(ptr, arr_a, arr_b, 1, size, test_obj.get()->Size());
		}
		...
		void TestForTypeSub()
		{
			...
			//Sub
			test_obj = ArithmeticBasic<T, dim>::Sub(in_a, in_b);
			test_obj.get()->CopyDataTo(ptr);
			control_add_sub<T>(ptr, arr_a, arr_b, -1, size, test_obj.get()->Size());
			//SubTo
			ArithmeticBasic<T, dim>::SubTo(test_obj, in_a, in_b);
			test_obj.get()->CopyDataTo(ptr);
			control_add_sub<T>(ptr, arr_a, arr_b, -1, size, test_obj.get()->Size());
		}
		...
		void TestForTypeMul()
		{
			...
			//Sub
			test_obj = ArithmeticBasic<T, dim>::Mul(in_a, in_b);
			test_obj.get()->CopyDataTo(ptr);
			control_mul<T>(ptr, arr_a, arr_b, size, test_obj.get()->Size());
			//SubTo
			ArithmeticBasic<T, dim>::MulTo(test_obj, in_a, in_b);
			test_obj.get()->CopyDataTo(ptr);
			control_mul<T>(ptr, arr_a, arr_b, size, test_obj.get()->Size());
		}
